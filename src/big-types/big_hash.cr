# require "crystal/hasher" # TODO: needed?

# A `BigHash` represents a collection of key-value mappings, similar to a dictionary.
#
...
#
# ```
# # Create a new BigHash for mapping String to Int32
# hash = BigHash(String, Int32).new
# hash["one"] = 1
# hash["two"] = 2
# hash["one"] # => 1
# ```
...
# it was inserted into the `BigHash` may lead to undefined behaviour. This can be
...
class BigHash(K, V)
  ...
  # ===========================================================================
  #
  # BigHash implements an open addressing collision resolution method:
  ...
  # Creates a new empty `BigHash` with a *block* for handling missing keys.
  #
  # ```
  ...
  # Creates a new empty `BigHash` with a *block* that handles missing keys.
  #
  # ```
  ...
  # Creates a new empty `BigHash` where the *default_value* is returned if a key is missing.
  #
  # ```
  ...
  private MAX_INDICES_SIZE_LINEAR_SCAN = 32_u64

  # Maximum number of `indices_size` for which we can represent `@indices`
  # as Pointer(UInt8).
  private MAX_INDICES_BYTESIZE_1 = 256_u64

  # Maximum number of `indices_size` for which we can represent `@indices`
  # as Pointer(UInt16).
  private MAX_INDICES_BYTESIZE_2 = 65536_u64

  # Maximum number of `indices_size` for which we can represent `@indices`
  # as Pointer(UInt32).
  private MAX_INDICES_BYTESIZE_4 = 4294967296_u64
  ...
  protected def find_entry_with_index(key) : {Entry(K, V), UInt64}?
  ...
  private def find_entry_with_index_linear_scan(key) : {Entry(K, V), UInt64}?
  ...
  private def do_compaction(rehash : Bool = false) : Nil
    ...
    each_entry_with_index do |entry, entry_index|
      ...
      if has_indices
        ...
        until get_index(index) == EMPTY_INDEX
          index = next_index(index)
        end
  ...
  # After this it's 1_u64 << 61, and with entries being UInt64
  # (8 bytes) it's 1_u64 << 63 of actual bytesize and the
  # next value would be 1_u64 << 64 which overflows `UInt64`.
  private MAXIMUM_INDICES_SIZE = 1_u64 << 61
  ...
  private def double_indices_size : Nil
    if indices_size == MAXIMUM_INDICES_SIZE
      raise "Maximum BigHash size reached"
    end
  ...
  private EMPTY_INDEX = UInt64::MAX

  # Gets from `@indices` at the given `index`.
  # Returns the index in `@entries` or `EMPTY_INDEX` if the slot is empty.
  private def get_index(index : UInt64) : UInt64
    # Check what we have: UInt8, Int16 or UInt32 buckets
    value = case @indices_bytesize
            when 1
              @indices[index].to_u64!
            when 2
              @indices.as(UInt16*)[index].to_u64!
            when 4
              @indices.as(UInt32*)[index].to_u64!
            else # 8
              @indices.as(UInt64*)[index].to_u64!
            end

    # Because we increment the value by one when we store the value
    # here we have to subtract one (and it might wrap around to EMPTY_INDEX).
    value &- 1
  end

  # Sets `@indices` at `index` with the given value.
  private def set_index(index : UInt64, value) : Nil
    # We actually store 1 more than the value because 0 means empty.
    value += 1

    # We also have to see what we have: UInt8, UInt16, UInt32 or UInt64 buckets.
    ...
    when 4
      ...
      @indices.as(UInt64*)[index] = value.to_u64!
    end
  end
  ...
  private def delete_index(index) : Nil
    ...
    i = index.to_u64
    set_index(i, EMPTY_INDEX)

    j = i
    while true
      j = next_index(j)
      entry_index = get_index(j)
      break if entry_index == EMPTY_INDEX
      ...
      set_index(j, EMPTY_INDEX)
      i = j
    end
  end

  # Returns the capacity of `@indices`.
  protected def indices_size : UInt64
    1_u64 << @indices_size_pow2
  end
  ...
  private def compute_indices_bytesize(size) : Int8
    ...
    when size <= MAX_INDICES_BYTESIZE_2
      2_i8
    when size <= MAX_INDICES_BYTESIZE_4
      4_i8
    else
      8_i8
    end
  ...
  private def malloc_indices(size : UInt64)
    Pointer(UInt8).malloc(indices_malloc_size(size))
  end

  # The actual number of bytes needed to allocate `@indices`.
  private def indices_malloc_size(size : UInt64) : UInt64
    size * @indices_bytesize.to_u64!
  end

  # Reallocates `size` number of indices for `@indices`.
  private def realloc_indices(size : UInt64)
  ...
  private def get_entry(index : UInt64) : Entry(K, V)
    @entries[index]
  end

  # Sets the entry in `@entries` at `index`.
  private def set_entry(index : UInt64, value) : Nil
    if index > size
      raise "OVERFLOW! index=#{index}, size=#{size}"
    end
    @entries[index] = value
  end
  ...
  # `@entries`.
  private def index_for_entry_index(entry_index : UInt64) : UInt64
  ...
  private def delete_entry(index : UInt64) : Nil
  ...
  private def delete_entry_and_update_counts(index : UInt64) : Nil
    delete_entry(index)
    @size -= 1
    @deleted_count += 1
  ...
  private def malloc_entries(size : UInt64)
    Pointer(Entry(K, V)).malloc(size)
  end

  private def realloc_entries(size : UInt64)
  ...
  private def next_index(index : UInt64) : UInt64
    fit_in_indices(index + 1)
  end

  # Fits a value inside the range of `@indices`
  private def fit_in_indices(value : UInt64) : UInt64
    ...
    value & ((1_u64 << @indices_size_pow2) - 1_u64)
  end
  ...
  protected def entries_size : UInt64
    @size + @deleted_count
  end
  ...
  private def key_hash(key) : UInt64
    if @compare_by_identity && key.responds_to?(:object_id)
      hash = key.object_id.hash.to_u64!
    else
      hash = key.hash.to_u64!
    end
    hash == 0 ? UInt64::MAX : hash
  end
  ...
  private def entry_matches?(entry, hash, key)
    ...
    {% if K == Bool ||
            ...
            K < Number::Primitive ||
            K < Enum %}
      entry.key == key && entry.hash != 0_u64
    {% else %}
  ...
  # ```
  # h = {"foo" => "bar", "baz" => "qux"}
  # h.values # => BigArray{"bar", "qux"}
  # ```
  def values : BigArray(V)
    to_a_impl &.value
  end

  # Returns a new `BigHash` with the keys and values of this hash and *other* combined.
  ...
  # # => {"foo" => "bar"}
  # ```
  def merge(other : BigHash(L, W)) : BigHash(K | L, V | W) forall L, W
    hash = BigHash(K | L, V | W).new
    hash.merge! self
    hash.merge! other
    hash
  end

  def merge(other : BigHash(L, W), & : L, V, W -> V | W) : BigHash(K | L, V | W) forall L, W
    hash = BigHash(K | L, V | W).new
    hash.merge! self
  ...
  protected def merge_into!(other : BigHash(K2, V2)) forall K2, V2
    {% unless K2 >= K && V2 >= V %}
  ...
  protected def merge_into!(other : BigHash(K2, V2), & : K, V2, V -> V2) forall K2, V2
    {% unless K2 >= K && V2 >= V %}
      {% raise "#{Hash(K, V)} can't be merged into #{Hash(K2, V2)}" %}
    {% end %}
  ...
  # Returns a new `BigHash` without the given keys.
  #
  # ```
  # {"a" => 1, "b" => 2, "c" => 3, "d" => 4}.reject("a", "c") # => {"b" => 2, "d" => 4}
  # ```
  def reject(*keys) : BigHash(K, V)
    hash = self.dup
    hash.reject!(*keys)
  end
  ...
  # Returns a new `BigHash` with the given keys.
  #
  # ```
  ...
  # ```
  def select(keys : Enumerable) : BigHash(K, V)
  ...
  # Returns new `BigHash` without `nil` values.
  #
  # ```
  ...
  # ```
  def transform_keys(& : K, V -> K2) : BigHash(K2, V) forall K2
  ...
  # ```
  def transform_values(& : V, K -> V2) : BigHash(K, V2) forall V2
  ...
  # Zips two arrays into a `BigHash`, taking keys from *ary1* and values from *ary2*.
  #
  # ```
  # Hash.zip(["key1", "key2", "key3"], ["value1", "value2", "value3"])
  # # => {"key1" => "value1", "key2" => "value2", "key3" => "value3"}
  # ```
  def self.zip(ary1 : BigArray(K), ary2 : BigArray(V))
    hash = BigHash(K, V).new
  ...
  def first_key : K
    entry = first_entry?
    entry ? entry.key : raise "Can't get first key of empty BigHash"
  end
  ...
  def first_value : V
    entry = first_entry?
    entry ? entry.value : raise "Can't get first value of empty BigHash"
  end
  ...
  def last_key : K
    entry = last_entry?
    entry ? entry.key : raise "Can't get last key of empty BigHash"
  end
  ...
  def last_value : V
    entry = last_entry?
    entry ? entry.value : raise "Can't get last value of empty BigHash"
  end
  ...
  # Empties a `BigHash` and returns it.
  #
  # ```
  # hash = {"foo" => "bar"}
  # hash.clear # => {}
  ...
  def proper_subset_of?(other : BigHash) : Bool
  ...
  def subset_of?(other : BigHash) : Bool
    return false if other.size < size
    all? do |key, value|
      other_value = other.fetch(key) { return false }
      other_value == value
  ...
  def superset_of?(other : BigHash) : Bool
    other.subset_of?(self)
  end

  # Returns `true` if *other* is a subset of `self` or equals to `self`.
  def proper_superset_of?(other : BigHash) : Bool
  ...
  # Returns a `BigArray` of `Tuple(K, V)` with key and values belonging to this BigHash.
  #
  # ```
  # h = {1 => 'a', 2 => 'b', 3 => 'c'}
  # h.to_a # => [{1, 'a'}, {2, 'b'}, {3, 'c'}]
  ...
  def to_a : BigArray({K, V})
    to_a(&.itself)
  end

  # Returns a `BigArray` with the results of running *block* against tuples with key and values
  # belonging to this BigHash.
  #
  # ```
  ...
  #
  # The order of the array follows the order the keys were inserted in the Hash.
  def to_a(&block : {K, V} -> U) : BigArray(U) forall U
    to_a_impl do |entry|
      yield ({entry.key, entry.value})
    end
  end
  ...
  # into the `BigHash` may lead to undefined behaviour. This method re-indexes the
  ...
  # ```
  def invert : BigHash(V, K)
    hash = BigHash(V, K).new(initial_capacity: @size)
    self.each do |k, v|
      hash[v] = k
    end
    hash
