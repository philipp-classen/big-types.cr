require "c/string"
require "./big_slice/sort"

# Like `Slice`, but not restricted to 32 bit addressing.
#
# A `BigSlice` is a `Pointer` with an associated size.
#
...
struct BigSlice(T)
  ...
  include Comparable(BigSlice)

  # Returns the size of this slice.
  #
  # ```
  # BigSlice(UInt8).new(3).size # => 3
  # ```
  getter size : UInt64
  ...
  #
  # slice = BigSlice.new(ptr, 3)
  # slice.size # => 3
  # slice      # => Bytes[97, 98, 99]
  #
  # String.new(slice) # => "abc"
  # ```
  def initialize(@pointer : Pointer(T), size : UInt64, *, @read_only = false)
    @size = size.to_u64
  end
  ...
  #
  # ```
  # slice = BigSlice(UInt8).new(3)
  # slice # => BigSlice[0, 0, 0]
  # ```
  def self.new(size : UInt64, *, read_only = false)
  ...
  #
  # ```
  # slice = BigSlice.new(3) { |i| i + 10 }
  # slice # => BigSlice[10, 11, 12]
  # ```
  ...
  #
  # ```
  # slice = BigSlice.new(3, 10)
  # slice # => BigSlice[10, 10, 10]
  # ```
  def self.new(size : UInt64, value : T, *, read_only = false)
  ...
  # ```
  def +(offset : Int) : BigSlice(T)
    check_size(offset)
    offset = offset.to_u64

    BigSlice.new(@pointer + offset, @size - offset, read_only: @read_only)
  end
  ...
  def +(other : BigSlice) : BigSlice
    BigSlice.join({self, other})
  end

  # Returns a new slice that has the elements from *slices* joined together.
  #
  # ```
  # BigSlice.join([Slice[1, 2], Slice[3, 4, 5]])        # => BigSlice[1, 2, 3, 4, 5]
  # BigSlice.join({Slice[1], Slice['a'], Slice["xyz"]}) # => BigSlice[1, 'a', "xyz"]
  # ```
  #
  # See also: `#+(other : Slice)`.
  def self.join(slices : Indexable(Slice | BigSlice)) : BigSlice
    total_size = slices.sum(&.size.to_u64)
  ...
  #
  # ```
  # slice = BigSlice.new(5) { |i| i + 10 }
  # slice # => BigSlice[10, 11, 12, 13, 14]
  #
  # slice[1, 3]?   # => BigSlice[11, 12, 13]
  # slice[1, 33]?  # => nil
  # slice[-3, 2]?  # => BigSlice[12, 13]
  # slice[-3, 10]? # => nil
  # ```
  def []?(start : Int, count : Int) : BigSlice(T)?
    ...
    BigSlice.new(@pointer + start, count, read_only: @read_only)
  end
  ...
  def map(*, read_only = false, & : T -> _)
    BigSlice.new(size, read_only: read_only) { |i| yield @pointer[i] }
  end

  # :inherit:
  #
  # Raises if this slice is read-only.
  def map_with_index!(offset = 0, & : T, UInt64 -> _) : self
    check_writable
  ...
  # from there.
  def map_with_index(offset = 0, *, read_only = false, & : (T, UInt64) -> _)
    BigSlice.new(size, read_only: read_only) { |i| yield @pointer[i], i + offset }
  end
  ...
  #
  # Raises if this slice is read-only.
  def fill(*, offset : Int = 0, & : UInt64 -> T) : self
    check_writable
    super { |i| yield i }
  end
  ...
  #
  # Raises if this slice is read-only.
  def fill(start : Int, count : Int, & : UInt64 -> T) : self
    check_writable
  ...
  #
  # Raises if this slice is read-only.
  def fill(range : Range, & : UInt64 -> T) : self
    check_writable
  ...
  # ```
  # src = BigSlice['a', 'a', 'a']
  # dst = BigSlice['b', 'b', 'b', 'b', 'b']
  # src.copy_to dst
  # dst             # => BigSlice['a', 'a', 'a', 'b', 'b']
  ...
  # Returns a new `BigSlice` pointing at the same contents as `self`, but
  ...
  # bytes = BigSlice[0x01, 0x02, 0x03, 0x04, 0xFF, 0xFE]
  # bytes.unsafe_slice_of(Int8)  # => BigSlice[1_i8, 2_i8, 3_i8, 4_i8, -1_i8, -2_i8]
  # bytes.unsafe_slice_of(Int16) # => BigSlice[513_i16, 1027_i16, -257_i16]
  # bytes.unsafe_slice_of(Int32) # => BigSlice[0x04030201]
  # ```
  def unsafe_slice_of(type : U.class) : BigSlice(U) forall U
    BigSlice.new(to_unsafe.unsafe_as(Pointer(U)), bytesize.to_u64 // sizeof(U), read_only: @read_only)
  end
  ...
  # ```
  # # assume little-endian system
  # ints = BigSlice[0x01020304, 0x05060708]
  # bytes = ints.to_unsafe_bytes # => BigSlice[0x04, 0x03, 0x02, 0x01, 0x08, 0x07, 0x06, 0x05]
  # bytes[2] = 0xAD
  # ints # => BigSlice[0x01AD0304, 0x05060708]
  # ```
  def to_unsafe_bytes : BigSlice(UInt8)
  ...
  def hexstring : String
    {% unless T == UInt8 %}
      {% raise "Can only call `#hexstring` on BigSlice(UInt8), not #{@type}" %}
    {% end %}
  ...
  def hexdump : String
    {% unless T == UInt8 %}
      {% raise "Can only call `#hexdump` on BigSlice(UInt8), not #{@type}" %}
    {% end %}
    ...
    String.new(str_size) do |buf|
      ...
      while pos < size
        ...
        hexdump_line(BigSlice.new(buf + offset, {77, str_size - offset}.min), pos)
        pos += 16
        offset += 77
      end
  ...
  #
  # `self` must be a `BigSlice(UInt8)`. To call this method on other `Slice`s,
  # `#to_unsafe_bytes` should be used first.
  #
  # This method is specially useful for debugging binary data and
  # incoming/outgoing data in protocols.
  ...
  # ```
  def ==(other : BigSlice(U)) : Bool forall U
  ...
  def pretty_print(pp) : Nil
    prefix = T == UInt8 ? "BigBytes[" : "BigSlice["
    pp.list(prefix, self, "]")
  end

  def to_a
    BigArray(T).build(@size) do |pointer|
  ...
  # ```
  # a = BigSlice[3, 1, 2]
  # a.unstable_sort # => BigBigSlice[1, 2, 3]
  # a               # => BigSlice[3, 1, 2]
  # ```
  #
  ...
  #
  # ```
  # slice = BigSlice[3, 1, 2]
  # slice.sort!
  # slice # => BigSlice[1, 2, 3]
  # ```
  #
  ...
  # Raises `ArgumentError` if the comparison between any two elements returns `nil`.
  def unstable_sort! : self
    BigSlice.intro_sort!(to_unsafe, size)
  ...
  def sort!(&block : T, T -> U) : self forall U
    ...
    BigSlice.merge_sort!(self, block)
  ...
  def unstable_sort!(&block : T, T -> U) : self forall U
    ...
    BigSlice.intro_sort!(to_unsafe, size, block)
  ...
  #
  # ```
  # a = BigSlice["apple", "pear", "fig"]
  # b = a.sort_by { |word| word.size }
  # b # => BigSlice["fig", "pear", "apple"]
  # a # => BigSlice["apple", "pear", "fig"]
  # ```
  #
  ...
  # a # => BigSlice["fig", "pear", "apple"]
  # ```
  def sort_by!(&block : T -> _) : BigSlice(T)
  ...
  def unstable_sort_by!(&block : T -> _) : BigSlice(T)
